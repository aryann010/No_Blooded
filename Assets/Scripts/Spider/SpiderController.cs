...
public class SpiderController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, player.transform.position) < attackRange) attackEvent();
    }
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (shot != null)
        {
            ...
            {
                dieEvent();
                StartCoroutine(delayInDeath());
                player.forScoreMultiplier();
            }
            else
                takeHitEvent();
        }
    }
    IEnumerator delayInDeath()
    {
        yield return new WaitForSeconds(3);
        Spawner.Instance.returnToPool(this);
    }
    public void attackedPlayer()=>player.updateScoreMethod();
}
